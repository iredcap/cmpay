...
class Orders extends BaseLogic
{
    ...
    public function getOrdersAllStat($where = []){
        return [
            'fees' => $this->getInfo($where,"COALESCE(sum(amount),0) as total,COALESCE(sum(if(status=2,amount,0)),0) as paid,COALESCE(sum(user_in),0) as user,COALESCE(sum(agent_in),0) as agent,COALESCE(sum(platform_in),0) as platform")
        ];
    }
    ...
    public function getWelcomeStat($where = []){
        ...
        return [
            'order' => $this->getInfo($where,"count(id) as total,count(if(status=2,true,null)) as success,count(if(status=1,true,null)) as wait,count(if(status=0,true,null)) as failed,COALESCE(sum(amount),0) as fees,COALESCE(sum(if(status=1,amount,0)),0) as unpaid,COALESCE(sum(if(status=2,amount,0)),0) as paid"),
            'user'  => $this->logicUser->getInfo($where,"count(uid) as total,count(if(is_verify=0,true,null)) as failed", $order, $paginate = false),
            'cash' => $this->logicBalanceCash->getInfo($where,'count(id) as total,count(if(status=2,true,null)) as success,count(if(status=1,true,null)) as wait,COALESCE(sum(if(status=0,amount,0)),0) as failed', $order, $paginate = false),
            'fees' => $this->getInfo($where,"COALESCE(sum(amount),0) as total,COALESCE(sum(if(status=2,amount,0)),0) as paid")
        ];
    }
    ...
    public function getOrdersMonthStat($where = []){
        $this->group = 'month';
        return $this->getList($where,"count(id) as total_orders,COALESCE(sum(`amount`),0) as total_amount,FROM_UNIXTIME(create_time,'%m') as month",false,false);
    }
    ...
    public function getOrderUserStat($where = [],$field = "uid,count(uid) as total_orders,COALESCE(sum(amount),0) as total_fee_all,COALESCE(sum(if(status=1,amount,0)),0) as total_fee_dis,COALESCE(sum(if(status=2,amount,0)),0) as total_fee_paid",$order = 'create_time desc', $paginate = 15){
        $this->group = 'uid';
        return $this->getList($where,$field, $order, $paginate = false);
    }
    ...
    public function pushOrderNotify($order_id = ''){
        ...
        //加入队列
        $result = $this->pushJobDataToQueue('AutoOrderNotify' , $order , 'AutoOrderNotify');
        if ($result){
            $returnmsg = [ 'code' =>  CodeEnum::SUCCESS, 'msg'  => '推送队列成功'];
        }else{
            $returnmsg = [ 'code' =>  CodeEnum::ERROR, 'msg'  => '推送队列失败'];
        }
    ...
    public function setOrderValue($where = [], $field = 'cnl_id', $value = ''){
        $this->setFieldValue($where, $field, $value);
    }
}